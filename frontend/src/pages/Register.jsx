...
export default function Register() {
  ...
  const handleContinueToStep3 = (e) => {
    ...
    if (!fullName || !companyName || !phone) {
      showMessage('Please fill in all basic details.', 'error');
      return;
    }
    setStep(3);
  };
  ...
  return (
    <div className="min-h-screen flex font-sans selection:bg-[#246dff] selection:text-white">
      ...
      {/* Right Pane - Form */}
      <div className="w-full lg:w-1/2 flex items-center justify-center bg-white/60 dark:bg-black/50 backdrop-blur-2xl relative py-12">
        {/* Theme Toggle */}
        <button
          onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
          className="absolute top-6 right-6 p-2 rounded-full cursor-pointer border border-gray-200 dark:border-[#333] text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
        >
        ...
        <div className="w-full max-w-[420px] px-6">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium text-sm">
              {step === 1 ? 'No credit card required.' : step === 2 ? 'We\'ll customize your workspace.' : 'Almost there! Create a password.'}
            </p>
          </div>
          ...
          <div className="flex gap-2 mb-8 justify-center">
            {[1, 2, 3].map((s) => (
              <div key={s} className={`h-1 rounded-full transition-all duration-300 ${s <= step ? 'w-8 bg-[#246dff]' : 'w-4 bg-gray-200 dark:bg-gray-800'}`} />
            ))}
          </div>
          ...
          {step === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <button onClick={() => setStep(1)} className="flex items-center gap-2 text-xs text-gray-500 hover:text-gray-900 dark:hover:text-white mb-6 font-semibold">
              ...
              <form onSubmit={handleContinueToStep3} className="space-y-4">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-tight mb-1 ml-1">Company Name</label>
                  <input
                    required
                    type="text"
                    value={companyName}
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-tight mb-1 ml-1">Phone Number</label>
                  <input
                    required
                    type="tel"
                    value={phone}
          ...
          {step === 3 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <button onClick={() => setStep(2)} className="flex items-center gap-2 text-xs text-gray-500 hover:text-gray-900 dark:hover:text-white mb-6 font-semibold">
                <ArrowLeft className="w-3.5 h-3.5" /> Back
              </button>

              <form onSubmit={handleRegister} className="space-y-4">
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-tight mb-1 ml-1">Business Type</label>
                  <select
                    value={businessType}
                    onChange={(e) => setBusinessType(e.target.value)}
                    className="block w-full px-3 py-2.5 border border-gray-200 dark:border-[#333] dark:bg-[#111] dark:text-white rounded text-gray-900 focus:outline-none focus:ring-1 focus:ring-[#246dff] transition-all text-sm cursor-pointer"
                  >
